...
//        PcrInitData.INSTANCE.initData();
        String princessId = PcrInitData.INSTANCE.getNameMap().get(name);
        if (princessId == null) {
    ...
    public Message addNickName(User sender, MessageChain messageChain, Contact subject) throws Exception {
        ...
        Map<String, String> nameMap = PcrInitData.INSTANCE.getNameMap();
...
//        PcrInitData.INSTANCE.initData();
        String character = PcrInitData.INSTANCE.getNameMap().get(name);
        if (character == null) {
            return new PlainText("无体检数据");
        } else {
            PcrPrincess pcrPrincess = PcrInitData.INSTANCE.getPrincessMap().get(character);
